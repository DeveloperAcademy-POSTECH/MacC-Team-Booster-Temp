...
class GeneralAPIManger {
    ...
    static func request<T: Codable>(for target: GeneralAPI, type: T.Type, completion: @escaping ((Result<T, MoyaError>) -> Void)) {
    ...
    static func request(for target: GeneralAPI, completion: @escaping ((Result<Int, MoyaError>) -> Void)) {
        let provider = MoyaProvider<GeneralAPI>()
