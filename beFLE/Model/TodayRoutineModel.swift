...
struct TodayRoutineModel {
    ...
    func fetchTodayRoutines(_ completion: @escaping ((ResponseGetUsersRoutines) -> Void)) {
        #if DEBUG
        let date = "2023-11-22"
        #else
        ...
        GeneralAPIManger.request(for: .GetUsersRoutines(date: date), type: [InfluencerRoutine].self) { result in
            switch result {
